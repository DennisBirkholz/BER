...
class BER_Type_Sequence implements ArrayAccess, BER_Type {
	...
	public function parse(&$data, $pos = 0, $length = null) {
		...
		if (count($me::$definition) === 0) {
			$this->value = BER::parse(BER::substr($data, $pos, $length));
			return;
		}
		
		if (count($elements) > count($me::$definition)) {
			throw new Exception('More elements found for this sequence, got #' . count($elements) . ', only #' . count($me::$definition) . ' allowed.');
		}
		
		foreach ($me::$definition AS $var => $classes) {
			$optional = false;
			$default = null;
	...
	public function encode() {
		...
		foreach ($me::$definition AS $var => $class) {
			$optional = false;
			$default = null;
			...
			if ((!is_array($class)) && ($class::TYPE === BER::TYPE_PRIMITIVE) && (!$this->$var instanceof $class)) {
				$v = $this->$var;
				$this->$var = new $class();
				$this->$var->init($v);
			}
	...
	final public function __get($key) {
		...
		if (isset($this->$key)) {
			...
			if (($class::TYPE === BER::TYPE_PRIMITIVE) && isset($this->$key->value)) {
		...
		if (is_array($me::$definition[$key]) && isset($me::$definition[$key]['default'])) {
			return $me::$definition[$key]['default'];
		}
	...
	final public function __set($key, $value) {
		...
		if (!isset($me::$definition[$key])) {
			throw new InvalidArgumentException('Field "' . $key . '" not allowed in sequence of type "' . $me . '".');
		}
		
		$classes = $me::$definition[$key];
	...
	public function offsetExists($key) {
		$me = get_class($this);
		return isset($me::$definition[$key]);
	}
